...
public class ATMNew2 {
	...
	public void withdraw()
	{
		...
		/*amountBeans.add(new AmountBean(20,20,400));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		
		amountBeans.add(new AmountBean(5,1,5));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		amountBeans.add(new AmountBean(1,1,1));
		...
		else
		{
			...
			Set<Double> denominations= reverseSortedMap.keySet();
			 ...
			 // 1. get Amount
			 // 2.loop through avalable denomination no
			 // 3. Compare amount NOTE : This is ony for "Calculation no disbusrment"
					   // amount (remaining) to be disbursed = no of donomi* amount
					//if (amount (remaining) to be disbursed = amount (remaining) to be requested
						// DISBURSE 
							// get the avaible Amount,NO and Denomiation to be disbursed
							// Minus it
							// Update it // new amountbean
							// STATUS SUCESS 
					//amount (remaining) to be disbursed  > amount (remaining) to be requested
							// get the avaible Amount,NO and Denomiation to be disbursed
							// Update it
							// Minus it and set 
					    			//amount (remaining) to be requested = amount (remaining) to be requested - no of donomi* amount
					    		// STATUS RETRY 
					    		// countinue until you get disbursement is 
			// 4.  Find the demo and no and subtract 
			// 5.  Reset
					
			//
			//reverseSortedMap.containsKey(key)
	 ...
	 /*
			tsSorteds.forEach(denominations -> {	
				System.out.print(" denomo"+denominations.doubleValue());
				Double sumOfdenominaation = reverseSortedMap.get(denominations.doubleValue()).stream().collect(Collectors.summarizingDouble(AmountBean::getNoofnotes)).getSum();
				System.out.println(" sumOfdenominaation "+sumOfdenominaation);
...
/*
 collect(Collectors.summarizingDouble(AmountBean::getNoofnotes)).getSum();			 		
 */
			/* for(int i=0;i<summedDepositCountbyDenomiation;i++)
			 {
				 reverseSortedMap.
			 }
			 */
			...
			// lab.forEach((k, v) -> System.out.println((k + ":" + v.stream().collect(collector))));
			
		}
		//List<AmountBean> filteredBean =amountBeans.stream().filter(x -> x.getDenomonation() == denominations.intValue()).collect(Collectors.toList());
	//	List<AmountBean> filteredBean =amountBeans.stream().collect(Collectors.toMap(amountBeans.g, amountBeans)) ;
	...
	public void deposit()
	{
		...
		do {
			...
			tsSorteds.forEach(denominations -> {
				Double transamount = amountBeans.stream()
